...
def cleanup_test_directory(basedir, test_name):
    ...
    for file in glob.glob(os.path.join(basedir, "test", f"do_{test_name}", "*.qv")):
        os.remove(file)
    for file in glob.glob(os.path.join(basedir, "test", f"do_{test_name}", "*.pdb")):
        os.remove(file)
    for file in glob.glob(os.path.join(basedir, "test", f"do_{test_name}", "*.sc")):
        os.remove(file)
    for file in glob.glob(os.path.join(basedir, "test", f"do_{test_name}", "*.csv")):
        os.remove(file)
    if os.path.exists(os.path.join(basedir, "test", f"do_{test_name}", "split")):
        shutil.rmtree(
            os.path.join(basedir, "test", f"do_{test_name}", "split"),
            ignore_errors=True,
        )
    if os.path.exists(os.path.join(basedir, "test", f"do_{test_name}", "input_pdbs")):
        shutil.rmtree(
            os.path.join(basedir, "test", f"do_{test_name}", "input_pdbs"),
            ignore_errors=True,
        )
